...
系统通过拟人化的 **King (规划者)**、**Lord (管理者)**、**Knight (执行者)** 分层架构，结合 **Roundtable (圆桌会议)** 协同机制，实现业务的自动拆解、执行与自我治理。
...
└─────────────────────────────────────────────────────────────────────────┘
        ...
        ▼                           ▼                           ▼
┌───────────────┐  ┌─────────────────────────────────┐  ┌───────────────┐
│Governance(王冠)│  │      Domain Plane (领地)        │  │Collab.(圆桌)  │
│               │  │                                 │  │               │
│ ┌───────────┐ │  │ ┌─────────┐      ┌─────────┐   │  │ ┌───────────┐ │
│ │  👑 King  │ │  │ │🏰Lord A │      │🏰Lord B │   │  │ │ ⚖️ 议长   │ │
│ │ (核心领域) │ │  │ │(Finance)│ ... │ (Tech)  │   │  │ └───────────┘ │
│ └───────────┘ │  │ └─────────┘      └─────────┘   │  │ ┌───────────┐ │
│ ┌───────────┐ │  │         │              │       │  │ │ 📝 会议纪要│ │
│ │📕通讯录   │ │  │         └──────┬───────┘       │  │ └───────────┘ │
...
| Orchestration Plane | 全局编排与生命周期管理 | Queen (神经中枢) |
| Governance Plane | 组织治理与跨域协调 | King, 通讯录, 策略引擎 |
| Domain Plane | 业务领域执行 | Lords, Knights |
| Collaboration Plane | 多方协同决策 | 议长, 会议纪要 |
...
|:-----|:---------|:---------|:-------------|
| King | 核心领域 | 组织治理、领域划分、资源调度、跨域协调 | `.realm/` (全局) |
| Lord | 业务领域 | 任务规划、任务执行、业务处理、知识沉淀 | `.realm/fiefdoms/{domain}/` |
...
### 4.1 治理层：王室双星 (The Royal Couple)

#### 4.1.1 King Agent (国王)

#### 王室双星：最小化系统的核心

**King + Queen 构成 Silicon Realm 的最小可运行系统**。即使没有任何 Lord，王室双星也能独立运作，处理所有业务。Lord 只是随着业务复杂度增长，从 King 的职责中拆分出来的业务领域。
...
King 是 **核心领域 (Core Domain)** 的 Agent，是王国的"灵魂"与"大脑"。
...
Queen 是王国的"躯体"与"神经系统"，负责将 King 的意志转化为行动。
...
| 职能 | 描述 | 隐喻 |
|:-----|:-----|:-----|
| 容器生命周期 | 启动、停止、重启所有 Agent 容器 | 心跳与呼吸 |
| 消息路由 | 监听 Redis Streams，分发消息 | 神经传导 |
| 沙箱编排 | 为 Knight 创建隔离的沙箱环境 | 肌肉协调 |
| 资源管控 | 执行配额限制，超支熔断 | 能量分配 |
| 记忆索引 | 将会话摘要向量化存入 Qdrant | 记忆固化 |
| 健康监控 | 监控所有 Agent 健康状态 | 免疫系统 |

#### 4.1.3 王室双星的互补关系

```
┌─────────────────────────────────────────────────────────────┐
│                    王室双星 (The Royal Couple)               │
│                                                             │
│   👑 King (灵魂/大脑)         👸 Queen (躯体/神经)          │
│   ├── 思考：我要做什么？      ├── 感知：发生了什么？        │
│   ├── 决策：应该怎么做？      ├── 执行：让它发生！          │
│   ├── 规划：未来走向哪？      ├── 协调：资源如何调配？      │
│   └── 治理：组织如何运作？    └── 保障：系统如何稳定？      │
│                                                             │
│   King 没有 Queen：有想法但无法执行（空想家）              │
│   Queen 没有 King：能执行但不知方向（无头苍蝇）            │
│                                                             │
│   两者相辅相成，缺一不可，共同构成最小可运行系统           │
└─────────────────────────────────────────────────────────────┘
```
...
### 4.3 协同层：圆桌会议 (The Roundtable)

系统的"议事厅"，仅限 King 和 Lords 参与。
...
| 角色 | 隐喻 | 职责 |
|:-----|:-----|:-----|
| 👑 King | 灵魂/大脑 | 思考、决策、规划、治理 |
| 👸 Queen | 躯体/神经 | 感知、执行、协调、保障 |

**King + Queen = 最小可运行系统**，两者相辅相成，缺一不可。
...
| 特性 | 价值 |
|:-----|:-----|
| 王室双星 | King (灵魂) + Queen (躯体) 构成最小系统，互补共生 |
...
| v2.10 | Queen 作为神经中枢：提升 Queen 地位，详细架构设计 |
