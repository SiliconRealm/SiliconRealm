...
#### Queen：王国的神经中枢 (The Nerve Center)

Queen 是整个 Silicon Realm 的 **核心编排引擎**，是王国真正的"心脏"和"神经中枢"。所有 Agent（King、Lord、Knight）的生命周期都由 Queen 管理。

| 属性 | 描述 |
|------|------|
| **本体** | 高性能系统服务（Go / Rust / Python） |
| **形态** | 王国的"神经中枢"，极低延迟的常驻守护进程 |
| **地位** | **最核心组件**，所有 Agent 的生命周期管理者 |
| **设计原则** | 高可用、低延迟、无状态（状态存储在 Redis/DB） |
...
| 职能 | 描述 | 重要性 |
|------|------|--------|
| **容器生命周期** | 启动、停止、重启 King/Lord/Knight 容器 | ⭐⭐⭐⭐⭐ |
| **消息路由** | 监听 Redis Streams，分发消息到目标 Agent | ⭐⭐⭐⭐⭐ |
| **沙箱编排** | 为 Knight 创建隔离的沙箱环境 | ⭐⭐⭐⭐⭐ |
| **资源管控** | 执行配额限制，超支熔断 | ⭐⭐⭐⭐ |
| **记忆索引** | 将会话摘要向量化存入 Qdrant | ⭐⭐⭐⭐ |
| **健康监控** | 监控所有 Agent 健康状态，自动恢复 | ⭐⭐⭐⭐ |
| **日志聚合** | 收集所有 Agent 日志，统一存储 | ⭐⭐⭐ |

**Queen 与其他组件的关系**：

| 关系 | 描述 |
|------|------|
| **Queen → King** | 启动 King 容器，转发用户消息，监控健康状态 |
| **Queen → Lord** | 启动/停止 Lord 容器，转发任务消息，管理 Topic 会话 |
| **Queen → Knight** | 创建沙箱环境，执行资源限制，任务完成后销毁 |
| **Queen → Redis** | 监听消息队列，存储状态，协调分布式锁 |
| **Queen → Qdrant** | 索引记忆向量，支持 RAG 查询 |
| **Queen → Docker** | 通过 Docker SDK 管理所有容器 |

**Queen 的高可用设计**：

| 策略 | 描述 |
|------|------|
| **无状态设计** | Queen 本身无状态，所有状态存储在 Redis/DB |
| **多实例部署** | 可部署多个 Queen 实例，通过 Redis 协调 |
| **故障恢复** | Agent 容器崩溃时自动重启 |
| **优雅降级** | 资源不足时优先保证 King 和高优先级 Lord |
...
class Queen:
    ...
    async def run(self):
        ...
        await asyncio.gather(
            self.message_router(),      # 消息路由
            self.container_manager(),   # 容器管理
            self.health_monitor(),      # 健康监控
            self.memory_indexer(),      # 记忆索引
            self.resource_guard(),      # 资源管控
        )
    
    async def message_router(self):
        """监听并路由消息"""
        async for msg in self.redis.xread_group('realm:messages'):
            target = msg['to']
            await self.ensure_agent_running(target)
            await self.deliver_message(target, msg)
    
    async def spawn_knight(self, parent: str, task: dict) -> Knight:
        """为父 Agent 创建 Knight 沙箱"""
        # 检查配额
        if not self.check_quota(parent):
            raise QuotaExceeded(parent)
        ...
        container = self.docker.containers.run(
            ...
            volumes={task['workspace']: {'bind': '/workspace', 'mode': 'rw'}},
        )
        
        return Knight(container, parent, task)
```
...
│         意志与决策            ★ 执行与保障 ★               │
│                                                             │
│   注：Queen 是整个王国的神经中枢，所有 Agent 的生命周期     │
│       都由 Queen 管理。King 虽然是"国王"，但也是 Queen     │
│       管理的一个 Agent 容器。                               │
└─────────────────────────────────────────────────────────────┘
```
...
- v2.10: Queen as Nerve Center - Elevated Queen's role as the most critical component of the realm, managing lifecycle of all Agents (King/Lord/Knight), detailed Queen architecture with container orchestration, message routing, sandbox management, and high availability design
