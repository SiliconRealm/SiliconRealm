...
系统通过拟人化的 **King (规划者)**、**Lord (管理者)**、**Knight (执行者)** 分层架构，结合 **Roundtable (圆桌会议)** 协作机制，实现业务的自动拆解、执行与自我治理。Agent 通过 **Skills (技能)** 和 **MCP Tools (工具)** 获得能力，而非被外部系统调度。
...
| 能力 (Capability) | 赋予 Agent 执行能力 | 🛠️ Skills, 🔧 MCP Tools |
| 共享资源 (Shared Resource) | 跨 Agent 共享的知识和数据 | 📚 Academy, 📕 Address Book |
...
└ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ┘
                                    │
┌─────────────────────────────────────────────────────────────────────────┐
│                     Agent 能力层 (Skills & Tools)                        │
│                                                                         │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐  ┌────────────┐  │
│  │ 🛠️ Skills    │  │ 🔧 MCP Tools │  │ 🐳 Docker    │  │ 📡 Redis   │  │
│  │  (技能包)    │  │  (外部工具)   │  │   SDK        │  │  Streams   │  │
│  └──────────────┘  └──────────────┘  └──────────────┘  └────────────┘  │
└─────────────────────────────────────────────────────────────────────────┘
                                    │
┌─────────────────────────────────────────────────────────────────────────┐
│                        Knowledge Plane (Academy)                         │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐                  │
...
| Capability Layer | 赋予 Agent 执行能力 | 🛠️ Skills, 🔧 MCP Tools, Docker SDK |
...
**King 是 Silicon Realm 的最小可运行系统**。King 通过 Skills 和 MCP Tools 获得能力，自主完成决策和执行。即使没有任何 Lord，King 也能独立运作。Lord 只是随着业务复杂度增长，从 King 的职责中拆分出来的业务领域。
...
│   │   • 通过 Skills 获得领域能力                        │   │
│   │   • 通过 MCP Tools 操作外部系统                     │   │
│   │   • 通过 Docker SDK 启动 Lord/Knight 容器           │   │
│   │                                                     │   │
│   │   Agent 不被调度，而是自己调度资源                  │   │
│   └─────────────────────────────────────────────────────┘   │
│                              │                              │
...
│   │   Lord 与 King 同构，同样通过 Skills/Tools 获得能力 │   │
│   └─────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────┘
```
...
King 通过 Skills 和 MCP Tools 获得能力，而非被外部系统调度：

| 能力类型 | 来源 | 示例 |
|:---------|:-----|:-----|
| 领域知识 | Skills (技能包) | 领域分析、任务规划、代码审查 |
| 外部操作 | MCP Tools | Docker SDK、Redis 客户端、文件系统 |
| 容器管理 | Docker SDK Tool | 启动/停止 Lord 和 Knight 容器 |
| 消息通信 | Redis Streams Tool | 发送/接收跨 Agent 消息 |

#### 4.1.3 Address Book (通讯录)
...
| 启动 | 父 Agent 通过 Docker SDK Tool 启动 Knight 沙箱容器 |
| 执行 | Knight 在沙盒中执行任务 |
...
Agent 不被外部系统调度，而是通过 Skills 和 MCP Tools 自主获得能力：

| 能力来源 | 描述 |
|:---------|:-----|
| �️ Skills | 领域知识、推理策略、Prompt 模板 |
| � MCP Tools | Docker SDK、Redis 客户端、文件系统等外部操作能力 |

**King = 最小可运行系统**。King 通过工具自主管理 Lord 和 Knight。

### 角色架构

| 角色 | 来源 | 职责 |
|:-----|:-----|:-----|
| 👑 King | 核心 Agent | 决策、治理、跨域协调 |
...
| v3.5 | 移除 Canal：Agent 通过 Skills/Tools 自治，而非被外部系统调度 |
